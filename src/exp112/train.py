...
class RibonanzaDataModule(LightningDataModule):
    ...
    def __dataloader(self, mode: str = "train") -> DataLoader:
        ...
        if mode =="train":
            sampler = torch.utils.data.RandomSampler(dataset_len)
        else:
            sampler = torch.utils.data.SequentialSampler(dataset_len)
        len_sampler = LenMatchBatchSampler(
            sampler, 
            batch_size=self._batch_size if mode == "train" else self._batch_size * 4,
