...
COMMENT = """
    transformer baseline, mask using error v2, clustering groupkfold cv, postnorm
    """
SEQ_PATH = "./../../input/"
...
def main(args):
    ...
    for fold, (train_split, val_split) in enumerate(kf.split(np.arange(len(df) // 2))):
        if args.fold != fold:
            continue
        # train_split = np.where(df["fold"].iloc[: len(df) // 2] != fold)[0]
        # val_split = np.where(df["fold"].iloc[: len(df) // 2] == fold)[0]
        ...
        early_stop_callback = callbacks.EarlyStopping(monitor=f'val_loss', patience=10, verbose=True)
